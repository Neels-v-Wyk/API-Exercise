...
class CollectionEndpoint(RestEndpoint):
    ...
    async def get(self) -> Response:
        ...
        response = Response(
            status=200, body=self.resource.encode(body), content_type="application/json"
        )
...
class RestResource:
    ...
    def register(self, router: UrlDispatcher):
        router.add_route(
            "*",
            "/{people}".format(people=self.people),
            self.collection_endpoint.dispatch,
        )
        router.add_route(
            "*",
            "/{people}/{{uuid}}".format(people=self.people),
            self.instance_endpoint.dispatch,
        )

    def render(self, instance):
        return OrderedDict(
            (people, getattr(instance, people)) for people in self.properties
        )
